...
Retain the six-column desktop card layout and two-column mobile layout. Give it calmer spacing, clear name/ticker hierarchy, square muted placeholder art, and comfortable search/filter controls. The hero figure is a close crop of the Macintosh CRT (`frontend/public/images/crt-close.jpg`) showing the coin that most recently left its curve on Pons: the whole picture in the upper glass, never cropped, with a bold Garamond phosphor readout of ticker and market cap in the band beneath, or, without a picture, the readout alone (name in italic Garamond, ticker, market cap, 24 h change, age), with NO SIGNAL whenever there is no coin to show. The caption is only "Recently migrated". The collection lists only coins that have migrated, in two categories, Pons coins (the pons-wide feed) and Plum coins (the registry), sorted by newest or market cap; see `GET /api/pons/coins` and `/api/pons/spotlight` in [LAUNCH-API.md](LAUNCH-API.md). It is live data, so it also shows on mobile, replacing the earlier decorative Macintosh that mobile omitted.
