...
## CRT edge and reflection cleanup — September 7, 2026

- User rejected the preceding CRT effects as low quality. Removed all painted glow strokes, the blurred flat-color screen halo, casing wash, artificial glass glint, and 160 x 120 bloom canvas/copy. Kept the brighter exposure curve and original glass silhouette. A small 1.6-unit highlight blur at .22 alpha now runs directly on the full-resolution picture and is clipped within the glass, leaving the original bezel detail visible.
- Keyboard spill is constrained by both its existing footprint and a luminance mask from the original artwork. Bright key planes receive more tint than dark recesses and legends. This preserves source texture instead of covering it with a uniform translucent patch; it is an image-based approximation, not a geometric material/light model. No original pixels were edited.
- The preview server had stopped; restarted production preview on the same confirmed URL, `http://127.0.0.1:4173/`, with strict port selection. Rebuilt/reloaded and compared the same paused opening frame at 1440 x 900 and 390 x 844. The pale outline and casing wash are removed, the picture stays readable, and controls remain visible with no horizontal overflow. One visible canvas is present. Evidence: [desktop before](screenshots/crt-clean-before-desktop.png), [desktop after](screenshots/crt-clean-after-desktop.png), [mobile before](screenshots/crt-clean-before-mobile.png), [mobile after](screenshots/crt-clean-after-mobile.png). Aesthetic assessment remains pending.
- Build, lint, all 24 existing tests, and whitespace checks passed. Manual Play produced changing footage; Pause restored Play TV with sound off. No browser warnings/errors were captured. The playback controller, 30fps ceiling, 5Hz sampling, opt-in audio, and hidden-tab/reduced-motion behavior are retained. Removed the extra per-frame bloom copy; no new loop or dependency. No comparative GPU benchmark or physical-device validation. No deployment.

## Brighter CRT and reflected light — September 7, 2026

User subsequently rejected this treatment's edge/reflection quality. The following observations are historical implementation evidence, not design approval.
