...
| `GET /api/pons/coins` | Pons-wide: the coins that have left their curve, from the `bonded` view of one Mobula pulse call for the configured factory (Mobula returns 50 per view), spam-flagged rows dropped, newest graduation first, with market cap, 24 h change, a DexScreener `chart` link, `madeWithPlum` for registry members and `creatorTaxBps` (the registry row for a Plum coin, otherwise one `getLaunchedToken` read per coin kept for the process; `null` while a read fails). A logo Mobula stops sending is remembered. One snapshot cached 30 s in memory; failures return the last list as `stale` |
...
- **Creators.** One intent costs three RPC reads (`canLaunch`, `eth_estimateGas`, balance). The provider's throughput cap, not credits, is the limit: on the Free plan, 100 wallets preparing at the same instant first met Alchemy 429s, and with reads gated eight at a time and a 429 backing off 500 ms, 1 s, 2 s, 4 s the same burst prepared 100 of 100 in 3.9 s. On pay-as-you-go (10,000 CU/s, September 9) the gate is sixteen: 100 simultaneous creators, each signing in and preparing, took 0.7 s (1.0 s at eight), and 300 took 1.5 s, with no refusals. A read the provider still refuses records `provider_busy` on the intent, and the desk's Edit button prepares a fresh one.
...
- **Live factory events, September 9.** With an Alchemy HTTPS URL the server also opens the matching `wss://` socket (`PLUM_RPC_WS_URL` overrides it; without either, polling alone) and subscribes to the factory's logs (`watchFactory` in `chain/client.ts`). A `TokenLaunched` log wakes the intents worker at once, so a Plum launch is recorded within a second of mining rather than on the next tick; any other factory log (graduations, config changes) asks the Pons feed to read ahead of its 30 s TTL, now and again 20 s and 60 s later because Mobula indexes a graduation some seconds after the chain, with forced reads at least 15 s apart. The socket reconnects on its own; while it is down everything falls back to the polling cadence. Observed on the dev server: the feed's `retrievedAt` advanced at 20 s and 16 s intervals during a quiet minute, ahead of the TTL. The worker tick is 2 s (was 5), the desk polls a submitted intent every 2 s (was 3), Explore refreshes every 15 s (was 30), and protocol state is cached 15 s per token (was 30). Viewers still cost the provider nothing; the extra cost is Mobula pulse calls, at most four a minute under a stream of graduations instead of two.
- **Abuse, checked September 9.** The worker visits only intents with a submitted hash; prepared intents expire in one SQL statement, so a wallet spamming 30 quotes a minute (the per-wallet cap) cannot push real launches out of the worker's 200-intent pass. A hash the network has not seen is rechecked after 5, 10, 20, 40, then 60 s, and an intent holds at most three unsettled hashes, so invented hashes cannot turn the worker into an RPC amplifier. Candle windows are quantized and metered. The Alchemy key (Free plan, 300 CU/s cap on its billing page) sustained 40 `eth_call`s a second cleanly, with the dashboard meter peaking at 277 CU/s, and refused most of 80 a second; the gate ran at about that rate, so bursts queued rather than failed; see Creators for the pay-as-you-go figures.
