...
test('health and launch-config are public; writes need same-origin and a session', async () => {
  ...
  assert.deepEqual(settings.body.pairTokens.map((p: { symbol: string }) => p.symbol), ['ETH', 'NVDA'])
  assert.equal(settings.body.pairTokens[0].graduationThreshold, '4200000000000000000')
  assert.equal(settings.body.pairTokens[1].graduationThreshold, '41600000000000000000')
