...
first = Image.open(root / items[0]['file']).convert('RGB')
scale = first.height / items[0]['h']
out = Image.new('RGB', (first.width, round(items[-1]['total'] * scale)))
end = 0
for item in items:
    ...
    if y > end:
        raise ValueError(f'Capture gap: {end} to {y}')
...
assert end >= out.height
out.save(root / 'full-page-screenshots' / f'{name}-stitched.png')
out.thumbnail((900, 5000))
out.save(Path('work/2026-09-06-pons-infrastructure-design') / f'{name}-review.png')
